    ...
    module Shopify
      class Adapter
        ...
        def public_image_url
          ...
          @public_image_url = File.join(app_url, image_name)
        end
        ...
        def local_image
          ...
          image_name = File.basename(product.imagen)
          image_path = Rails.public_path.join(image_name)
          @local_image = File.open(image_path, "wb+")
        end

        def unstaged_local_image
          FileUtils.rm_f(local_image.path) if File.exist?(local_image.path)
        end
